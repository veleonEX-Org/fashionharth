...
export async function addFavorite(userId, itemId) {
    await pool.query("INSERT INTO favorites (user_id, item_id) VALUES ($1, $2) ON CONFLICT DO NOTHING", [userId, itemId]);
}
...
export async function saveLook(userId, look) {
    const result = await pool.query(`
...
export async function getSavedLooks(userId) {
    const result = await pool.query(`
